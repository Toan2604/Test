...
using DMS.Models;
using DMS.Service;
using DMS.Services.MGlobalUser;
using DMS.Service.MComment;
...
namespace DMS.Rpc.discussion
{
    ...
    public class DiscussionController : SimpleController
    {
        ...
        public async Task<List<Discussion_GlobalUserDTO>> SingleListGlobalUser([FromBody] Discussion_GlobalUserFilterDTO Discussion_GlobalUserFilterDTO)
        {
            GlobalUserFilter GlobalUserFilter = new GlobalUserFilter
            {
                ...
                RowId = new GuidFilter { NotEqual = CurrentContext.UserRowId }
            };
            List<GlobalUser> GlobalUsers = await GlobalUserService.List(GlobalUserFilter);

            List<Guid> StoreUserRowIds = new List<Guid>();
            if (Discussion_GlobalUserFilterDTO.StoreId != null && Discussion_GlobalUserFilterDTO.StoreId.HasValue)
            {
                var StoreIdFilter = Discussion_GlobalUserFilterDTO.StoreId;
                var StoreUsers = await StoreUserService.List(new StoreUserFilter
                {
                    Selects = StoreUserSelect.RowId,
                    Take = int.MaxValue,
                    Skip = 0,
                    StoreId = Discussion_GlobalUserFilterDTO.StoreId,
                });
                StoreUserRowIds = StoreUsers.Select(x => x.RowId).ToList();
            }
            GlobalUsers = GlobalUsers.Where(x => x.GlobalUserTypeId == GlobalUserTypeEnum.APPUSER.Id || StoreUserRowIds.Contains(x.RowId)).Take(10).ToList();

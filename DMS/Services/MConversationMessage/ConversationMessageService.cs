...
namespace DMS.Services.MConversationMessage
{
        ...
        public async Task<List<ConversationMessage>> List(ConversationMessageFilter ConversationMessageFilter)
        {
            try
            {
                ...
                if (GlobalUser != null)
                {
                    ...
                    IRestRequest RestRequest = new RestRequest("rpc/utils/conversation-message/read");
                    RestRequest.AddQueryParameter("ConversationId", $"{ConversationMessageFilter.ConversationId}");
                    RestRequest.AddQueryParameter("GlobalUserId", $"{GlobalUser.Id}");
                    RestRequest.AddHeader("Cookie", "Token=" + CurrentContext.Token);
                    IRestResponse<bool> result = RestClient.Post<bool>(RestRequest);
                }

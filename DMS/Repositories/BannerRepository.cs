...
namespace DMS.Repositories
{
    public interface IBannerRepository
    {
        ...
        Task<int> CountAll(BannerFilter BannerFilter);
    ...
    public class BannerRepository : IBannerRepository
    {
        ...
        private IQueryable<BannerDAO> DynamicFilter(IQueryable<BannerDAO> query, BannerFilter filter)
        {
            ...
            if (filter.MobileOrganizationId != null)
            {
                // Đối với mobile, chỉ nhìn thấy những thằng cha nó và chính nó.
                if (filter.MobileOrganizationId.Equal != null)
                {
                    OrganizationDAO OrganizationDAO = DataContext.Organization
                        .Where(o => o.Id == filter.MobileOrganizationId.Equal.Value && o.StatusId == 1).FirstOrDefaultWithNoLock();
                    if (OrganizationDAO != null)
                        query = query.Where(q => OrganizationDAO.Path.StartsWith(q.Organization.Path));
                }
                if (filter.MobileOrganizationId.NotEqual != null)
                {
                    OrganizationDAO OrganizationDAO = DataContext.Organization
                        .Where(o => o.Id == filter.MobileOrganizationId.NotEqual.Value && o.StatusId == 1).FirstOrDefaultWithNoLock();
                    if (OrganizationDAO != null)
                        query = query.Where(q => !OrganizationDAO.Path.StartsWith(q.Organization.Path));
                }
            }
            ...
            if (filter.OrganizationId != null)
            {
                ...
                {
                    OrganizationDAO OrganizationDAO = DataContext.Organization
                        .Where(o => o.Id == filter.OrganizationId.Equal.Value && o.StatusId == 1).FirstOrDefaultWithNoLock();
                    if (OrganizationDAO != null)
                ...
                {
                    OrganizationDAO OrganizationDAO = DataContext.Organization
                        .Where(o => o.Id == filter.OrganizationId.NotEqual.Value && o.StatusId == 1).FirstOrDefaultWithNoLock();
                    if (OrganizationDAO != null)
                        query = query.Where(q => !q.Organization.Path.StartsWith(OrganizationDAO.Path));
                }
                if (filter.OrganizationId.In != null)
                {
                    List<OrganizationDAO> OrganizationDAOs = DataContext.Organization
                        .Where(o => o.DeletedAt == null && o.StatusId == StatusEnum.ACTIVE.Id).ToListWithNoLock();
        ...
        public async Task<int> Count(BannerFilter filter)
        {
            ...
            Banners = OrFilter(Banners, filter);
            return await Banners.CountWithNoLockAsync();
        }

        public async Task<int> CountAll(BannerFilter filter)
        {
            IQueryable<BannerDAO> Banners = DataContext.Banner.AsNoTracking();
            Banners = DynamicFilter(Banners, filter);
            return await Banners.CountWithNoLockAsync();
        ...
        public async Task<List<Banner>> List(BannerFilter filter)
        {
            ...
            BannerDAOs = OrFilter(BannerDAOs, filter);

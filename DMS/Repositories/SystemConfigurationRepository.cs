...
namespace DMS.Repositories
{
    ...
    public class SystemConfigurationRepository : CacheRepository, ISystemConfigurationRepository
    {
        private DataContext DataContext;
        public SystemConfigurationRepository(DataContext DataContext, IRedisStore RedisStore, IConfiguration Configuration) : base(DataContext, RedisStore, Configuration)
        {
        ...
        {
            List<SystemConfigurationDAO> SystemConfigurationDAOs = await DataContext.SystemConfiguration.ToListWithNoLockAsync();
            foreach (SystemConfigurationDAO SystemConfigurationDAO in SystemConfigurationDAOs)
            {
                ...
                if (SystemConfigurationDAO.Code == nameof(SystemConfigurationEnum.ALLOW_UPDATE_APPROVED_PRICE_LIST))
                    SystemConfigurationDAO.Value = SystemConfiguration.ALLOW_UPDATE_APPROVED_PRICE_LIST.ToString();

                if (SystemConfigurationDAO.Code == nameof(SystemConfigurationEnum.START_WORKFLOW_BY_USER_TYPE))
                    SystemConfigurationDAO.Value = SystemConfiguration.START_WORKFLOW_BY_USER_TYPE.ToString();

                if (SystemConfigurationDAO.Code == nameof(SystemConfigurationEnum.USE_ELASTICSEARCH))
                    SystemConfigurationDAO.Value = SystemConfiguration.USE_ELASTICSEARCH.ToString();
            }
            await DataContext.SaveChangesAsync();
            await RemoveFromCache(nameof(SystemConfigurationRepository));
            return true;
        }

        public async Task<SystemConfiguration> Get()
        {
            string key = $"{nameof(SystemConfigurationRepository)}.{nameof(Get)}";
            SystemConfiguration SystemConfiguration = await GetFromCache<SystemConfiguration>(key);
            if (SystemConfiguration == null)
            {
                SystemConfiguration = new SystemConfiguration();
                List<SystemConfigurationDAO> SystemConfigurationDAOs = await DataContext.SystemConfiguration
                    .AsNoTracking()
                ...
                await SetToCache(key, SystemConfiguration);
            }

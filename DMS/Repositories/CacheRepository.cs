    ...
    public class CacheRepository
    {
        ...
        protected async Task<T> GetFromCache<T>(string key)
        {
            try
            {
                ...
                byte[] value = await Database.StringGetAsync(key);
                if (value == null)
                    return default(T);
                ...
                T cachedResult = MessagePackSerializer.Deserialize<T>(value, options);
                return cachedResult;
            }
            catch (Exception ex)
            {
        ...
        protected async Task SetToCache<T>(string key, T value)
        {
            ...
            try
            {
                ...
                var options = MessagePackSerializerOptions.Standard.WithResolver(resolver);
                byte[] json = MessagePackSerializer.Serialize(value, options);
                IDatabase Database = await RedisStore.GetDatabase();

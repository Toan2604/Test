...
namespace DMS.Repositories
{
    ...
    public class EsProductRepository : IEsProductRepository
    {
        ...
        private BoolQueryDescriptor<Product> DynamicQuery(BoolQueryDescriptor<Product> query, ProductFilter filter)
        {
            ...
            query = query.EsWhere(q => q.IsNew, filter.IsNew);
        ...
        private SourceFilterDescriptor<Product> DynamicSelect(ProductFilter ProductFilter)
        {
            ...
            if (ProductFilter.Selects.Contains(ProductSelect.VariationGrouping))
                Fields.Add(new Nest.Field(nameof(Product.VariationGroupings)));
            if (ProductFilter.Selects.Contains(ProductSelect.UsedVariationId))
                Fields.Add(new Nest.Field(nameof(Product.UsedVariationId)));
            if (ProductFilter.Selects.Contains(ProductSelect.ProductImageMappings))
                Fields.Add(new Nest.Field(nameof(Product.ProductImageMappings)));

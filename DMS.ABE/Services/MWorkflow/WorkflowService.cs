...
namespace DMS.ABE.Services.MWorkflow
{
    public interface IWorkflowService : IServiceScoped
    {
        ...
        Task<GenericEnum> Send(Guid RequestId, long WorkflowTypeId, long OrganiaztionId, Dictionary<string, string> Parameters, AppUser AppUser);
        Task<GenericEnum> Approve(Guid RequestId, long WorkflowTypeId, Dictionary<string, string> Parameters, AppUser AppUser);
        Task<GenericEnum> Reject(Guid RequestId, long WorkflowTypeId, Dictionary<string, string> Parameters, AppUser AppUser);
    }
    public class WorkflowService : IWorkflowService
    {
        ...
        /// <returns>Request State Enum</returns>
        public async Task<GenericEnum> Initialize(Guid RequestId, long WorkflowTypeId, long OrganizationId, Dictionary<string, string> Parameters, AppUser AppUser)
        {
            ...
            // Kiểm tra trong workflow definition chọn được có workflow step nào thoả mãn việc step NGUỒN là 1 trong các role của nv tạo đơn
            // Nếu có step NGUỒN thì workflow có thể được khởi tạo
            if (WorkflowDefinition.WorkflowSteps != null)
            {
                var Roles = AppUser.AppUserRoleMappings.Select(x => x.RoleId).ToList();
                ...
                foreach (WorkflowStep WorkflowStep in WorkflowDefinition.WorkflowSteps)
                {
                    ...
                    {
                        if (Roles.Contains(WorkflowStep.RoleId))
                            ShouldInit = true;
                    }
                }
                if (ShouldInit == false)
        ...
        private async Task<bool> StartStep(Guid RequestId, Dictionary<string, string> Parameters, AppUser AppUser)
        {
            RequestWorkflowDefinitionMapping RequestWorkflowDefinitionMapping = await UOW.RequestWorkflowDefinitionMappingRepository.Get(RequestId);
            ...
            else
            {
                ...
                var RoleIds = AppUser.AppUserRoleMappings.Select(x => x.RoleId).ToList();
                foreach (WorkflowStep WorkflowStep in WorkflowDefinition.WorkflowSteps)
                {
                    if (RoleIds.Contains(WorkflowStep.RoleId) &&
                        !WorkflowDefinition.WorkflowDirections.Any(d => d.ToStepId == WorkflowStep.Id))
                    {
        ...
        private async Task<bool> ApproveStep(Guid RequestId, Dictionary<string, string> Parameters, AppUser AppUser)
        {
            RequestWorkflowDefinitionMapping RequestWorkflowDefinitionMapping = await UOW.RequestWorkflowDefinitionMappingRepository.Get(RequestId);
            WorkflowDefinition WorkflowDefinition = await UOW.WorkflowDefinitionRepository.Get(RequestWorkflowDefinitionMapping.WorkflowDefinitionId);
            {
            ...
            if (WorkflowDefinition != null && RequestWorkflowStepMappings.Count > 0)
            {
                ...
                foreach (WorkflowStep WorkflowStep in WorkflowDefinition.WorkflowSteps)
                {
                    if (RoleIds.Contains(WorkflowStep.RoleId))
                    {
                        ...
                        if (StartRequestStep != null)
                        {
                            ...
                            StartRequestStep.AppUserId = AppUser.Id;
                            foreach (WorkflowDirection WorkflowDirection in WorkflowDefinition.WorkflowDirections)
                            {
                ...
                foreach (var WorkflowDirection in WorkflowDirections)
                {
                    ...
                    AppUserFilter AppUserFilter = new AppUserFilter
                    {
                        ...
                        Skip = 0,
                        Take = int.MaxValue,
                        Selects = AppUserSelect.Email | AppUserSelect.RowId
                    };
                    ...
                    //gửi thông báo
                    if (Parameters.TryGetValue("Type", out string value1) && value1 == nameof(DirectSalesOrder))
                    {
                        ...
                        if (Parameters.TryGetValue(nameof(DirectSalesOrder.Code), out string Code))
                        {
                            DirectSalesOrder.Code = Code;
                            foreach (var Recipient in appUsers)
                            {
                                GlobalUserNotification GlobalUserNotification = DirectSalesOrderTemplate.CreateAppUserNotification(StoreUser.RowId, Recipient.RowId, DirectSalesOrder, Store, NotificationType.WAIT);
                ...
                RabbitManager.PublishList(Mails, RoutingKeyEnum.MailSend.Code);
                return true;
            }
        ...
        private async Task<bool> RejectStep(Guid RequestId, Dictionary<string, string> Parameters, AppUser AppUser)
        {
            RequestWorkflowDefinitionMapping RequestWorkflowDefinitionMapping = await UOW.RequestWorkflowDefinitionMappingRepository.Get(RequestId);
            WorkflowDefinition WorkflowDefinition = await UOW.WorkflowDefinitionRepository.Get(RequestWorkflowDefinitionMapping.WorkflowDefinitionId);
            List<RequestWorkflowStepMapping> RequestWorkflowStepMappings = await UOW.RequestWorkflowStepMappingRepository.List(RequestId);
        ...
        /// <param name="Parameters"></param>
        /// <returns>Request State Enum</returns>
        public async Task<GenericEnum> Send(Guid RequestId, long WorkflowTypeId, long OrganiaztionId, Dictionary<string, string> Parameters, AppUser AppUser)
        {
            GenericEnum RequestState = await Initialize(RequestId, WorkflowTypeId, OrganiaztionId, Parameters, AppUser);
            if (RequestState == RequestStateEnum.APPROVED)
                return RequestStateEnum.APPROVED;
            bool IsStarted = await StartStep(RequestId, Parameters, AppUser);
            if (IsStarted)
            {
                RequestState = await Approve(RequestId, WorkflowTypeId, Parameters, AppUser);
                return RequestState;
            }
            return RequestStateEnum.APPROVED;
        }
        ...
        /// <param name="Parameters"></param>
        /// <returns>Request State Enum</returns>
        public async Task<GenericEnum> Approve(Guid RequestId, long WorkflowTypeId, Dictionary<string, string> Parameters, AppUser AppUser)
        {
            await ApproveStep(RequestId, Parameters, AppUser);
            GenericEnum RequestState = await EndStep(RequestId, Parameters);
            return RequestState;
        }

        public async Task<GenericEnum> Reject(Guid RequestId, long WorkflowTypeId, Dictionary<string, string> Parameters, AppUser AppUser)
        {
            await RejectStep(RequestId, Parameters, AppUser);
            GenericEnum RequestState = await EndStep(RequestId, Parameters);
            return RequestState;
        }
        ...
        {
            {
                RabbitManager.PublishSingle(WorkflowDefinition, RoutingKeyEnum.WorkflowDefinitionUsed.Code);
            }
        }

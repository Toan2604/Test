...
namespace DMS.ABE.Repositories
{
    ...
    public class LuckyDrawRepository : ILuckyDrawRepository
    {
        ...
        private async Task<IQueryable<LuckyDrawDAO>> DynamicFilter(IQueryable<LuckyDrawDAO> query, LuckyDrawFilter filter)
        {
            ...
            {
                if (filter.StoreId.Equal != null)
                {
                    StoreDAO StoreDAO = DataContext.Store
                        .Where(o => o.Id == filter.StoreId.Equal.Value).FirstOrDefault();
                    var StoreQuery = from q in query
                            join sm in DataContext.LuckyDrawStoreMapping on q.Id equals sm.LuckyDrawId
                            join s in DataContext.Store on sm.StoreId equals s.Id
                            where q.LuckyDrawTypeId == LuckyDrawTypeEnum.STORE.Id && s.Id == StoreDAO.Id
                            select q;
                    var StoreGroupingQuery = from q in query
                                     join sgm in DataContext.LuckyDrawStoreGroupingMapping on q.Id equals sgm.LuckyDrawId
                                     join sg in DataContext.StoreGrouping on sgm.StoreGroupingId equals sg.Id
                                     join ssgm in DataContext.StoreStoreGroupingMapping on sg.Id equals ssgm.StoreGroupingId
                                     where q.LuckyDrawTypeId == LuckyDrawTypeEnum.STOREGROUPING.Id && ssgm.StoreId == StoreDAO.Id
                                     select q;
                    var StoreTypeQuery = from q in query
                                     join stm in DataContext.LuckyDrawStoreTypeMapping on q.Id equals stm.LuckyDrawId
                                     join st in DataContext.StoreType on stm.StoreTypeId equals st.Id
                                     join s in DataContext.Store on st.Id equals s.StoreTypeId
                                     where q.LuckyDrawTypeId == LuckyDrawTypeEnum.STORETYPE.Id && s.Id == StoreDAO.Id
                                     select q;
                    query = query.Where(x => x.LuckyDrawTypeId, new IdFilter { Equal = LuckyDrawTypeEnum.ALLSTORE.Id });
                    query = query.Union(StoreQuery).Union(StoreGroupingQuery).Union(StoreTypeQuery).Distinct();
                }

            }
            if (filter.HasPrize.HasValue)
            {
                if (filter.HasPrize.Value == true)
                {
                    var LuckyDrawIds = DataContext.LuckyDrawWinner.Where(x => x.LuckyDrawRegistration.StoreId == filter.StoreId.Equal.Value).Select(x => x.LuckyDrawId).Distinct().ToList();
                    query = query.Where(x => x.Id, new IdFilter { In = LuckyDrawIds });
                }
            }
            if (filter.InNotAction.HasValue)
            {
                ...
                var LuckyDrawIds = query.Select(x => x.Id).Distinct().ToList();
                if (filter.InNotAction.Value == true)
                {
                    List<long> Ids = new List<long>();
                    foreach (var LuckyDrawId in LuckyDrawIds)
                    {
                        var TotalPrizes = LuckyDrawStructureQuery.Where(x => x.LuckyDrawId == LuckyDrawId).Sum(x => x.Quantity);
                        var UsedPrizes = LuckyDrawWinnerQuery.Count(x => x.LuckyDrawId == LuckyDrawId);
                        if (TotalPrizes <= UsedPrizes)
                        {
                            Ids.Add(LuckyDrawId);
                        }
                    }
                    var tempQuery1 = query.Where(x => x.Id, new IdFilter { In = Ids });
                    query = query.Where(q => q.StatusId == StatusEnum.INACTIVE.Id || //điều kiện 3: chương trình inactive
                                            Ids.Contains(q.Id) || //điều kiện 2: chương trình hết giải
                                            (q.Used == true && q.EndAt < StaticParams.DateTimeNow)); //điều kiện 1: chương trình đã sử dụng nhưng hết hạn
                }
                else if (filter.InNotAction.Value == false)
                {
                    List<long> Ids = new List<long>();
                    foreach (var LuckyDrawId in LuckyDrawIds)
                    {
        ...
        private async Task<IQueryable<LuckyDrawDAO>> OrFilter(IQueryable<LuckyDrawDAO> query, LuckyDrawFilter filter)
        {
            ...
            foreach (LuckyDrawFilter LuckyDrawFilter in filter.OrFilter)
            {
                ...
                if (LuckyDrawFilter.OrganizationId != null && LuckyDrawFilter.OrganizationId.HasValue)
                {
                    ...
                    {
                        OrganizationDAO OrganizationDAO = DataContext.Organization
                            .Where(o => o.Id == LuckyDrawFilter.OrganizationId.Equal.Value).FirstOrDefault();
                    ...
                    {
                        OrganizationDAO OrganizationDAO = DataContext.Organization
                            .Where(o => o.Id == LuckyDrawFilter.OrganizationId.NotEqual.Value).FirstOrDefault();
                        queryable = queryable.Where(q => !q.Organization.Path.StartsWith(OrganizationDAO.Path));
                    }
        ...
        public async Task<int> CountAll(LuckyDrawFilter filter)
        {
            ...
            return await LuckyDrawDAOs.CountAsync();
        }
        ...
        private async Task SaveReference(LuckyDraw LuckyDraw)
        {
            ...
            if (LuckyDraw.LuckyDrawWinners != null)
            {
                foreach (LuckyDrawWinner LuckyDrawWinner in LuckyDraw.LuckyDrawWinners)
                {
                    LuckyDrawWinnerDAO LuckyDrawWinnerDAO = new LuckyDrawWinnerDAO();
                    LuckyDrawWinnerDAO.Id = LuckyDrawWinner.Id;
                    LuckyDrawWinnerDAO.LuckyDrawId = LuckyDraw.Id;
                ...
                await DataContext.LuckyDrawWinner.BulkMergeAsync(LuckyDrawWinnerDAOs);
            }

...
namespace DMS.Tests.Rpc.lucky_draw
{
    public partial class LuckyDrawControllerFeature
    {
        ...
        {
            await Then_Id(LuckyDraw_LuckyDrawDTO);
            await Then_Code(LuckyDraw_LuckyDrawDTO);
            await Then_Name(LuckyDraw_LuckyDrawDTO);
            await Then_LuckyDrawTypeId(LuckyDraw_LuckyDrawDTO);
            await Then_OrganizationId(LuckyDraw_LuckyDrawDTO);
            await Then_RevenuePerTurn(LuckyDraw_LuckyDrawDTO);
            await Then_StartAt(LuckyDraw_LuckyDrawDTO);
            await Then_EndAt(LuckyDraw_LuckyDrawDTO);
            await Then_AvatarImageId(LuckyDraw_LuckyDrawDTO);
            await Then_ImageId(LuckyDraw_LuckyDrawDTO);
            await Then_Description(LuckyDraw_LuckyDrawDTO);
            await Then_StatusId(LuckyDraw_LuckyDrawDTO);
            await Then_Used(LuckyDraw_LuckyDrawDTO);
        }
        private async Task Then_Update_Success()
        {
            await Then_Update_Id(LuckyDraw_LuckyDrawDTO);
            await Then_Code(LuckyDraw_LuckyDrawDTO);
            await Then_Name(LuckyDraw_LuckyDrawDTO);
            await Then_LuckyDrawTypeId(LuckyDraw_LuckyDrawDTO);
            await Then_OrganizationId(LuckyDraw_LuckyDrawDTO);
        ...
        private async Task Then_Search_Success()
        {
            Assert.AreEqual(2, LuckyDraw_LuckyDrawDTOs.Count);
            Assert.AreEqual("Chương trình quay thưởng tháng 9 dành cho đại lý cấp 1", LuckyDraw_LuckyDrawDTOs[0].Name);
            Assert.AreEqual("Lucky_T9", LuckyDraw_LuckyDrawDTOs[0].Code);
            Assert.AreEqual(56, LuckyDraw_LuckyDrawDTOs[0].Id);
            Assert.AreEqual("Chương trình quay thưởng tháng 8 cho khách hàng Cấp 1", LuckyDraw_LuckyDrawDTOs[1].Name);
            Assert.AreEqual("Lucky_T8", LuckyDraw_LuckyDrawDTOs[1].Code);
            Assert.AreEqual(55, LuckyDraw_LuckyDrawDTOs[1].Id);
        }
        private async Task Then_SearchManyFilter_Success()
        {
            Assert.AreEqual(1, LuckyDraw_LuckyDrawDTOs.Count);
            Assert.AreEqual("inscryjebe", LuckyDraw_LuckyDrawDTOs[0].Name);
            Assert.AreEqual("w3r3wre", LuckyDraw_LuckyDrawDTOs[0].Code);
            Assert.AreEqual(28, LuckyDraw_LuckyDrawDTOs[0].Id);
            Assert.AreEqual(445, LuckyDraw_LuckyDrawDTOs[0].RevenuePerTurn);
        }
        private async Task Then_Update_Failed()
        {
            Assert.AreEqual("Không thể thay đổi đối tượng áp dụng", LuckyDraw_LuckyDrawDTO.Errors["luckyDrawType"]);
            Assert.AreEqual(32, LuckyDraw_LuckyDrawDTO.Id);
        }
